...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    const comments = await fetchCommentThreads(CommentItemType.GAME, params.slug)
    return NextResponse.json(comments)
  } catch (error) {
    return errorResponse(req, error, "AO BUSCAR COMENTÁRIOS DO GAME")
  }
}
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    if (payload.parentId) {
      ...
      if (
        !parentComment ||
        parentComment.itemType !== CommentItemType.GAME ||
    ...
    const [createdComment, updatedGame] = await prisma.$transaction([
      prisma.comment.create({
        data: {
          itemType: CommentItemType.GAME,
          itemId: game.slug,
    ...
    if (parentComment) {
      return NextResponse.json({
        ...
        totalCount: updatedGame.commentsCount,
      })
    }
    ...
    return errorResponse(req, error, "AO PUBLICAR COMENTÁRIO DO GAME")
  }
}
