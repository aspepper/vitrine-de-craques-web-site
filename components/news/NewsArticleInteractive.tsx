...
const COMMENTS_STORAGE_PREFIX = "vitrine:news:comments";
...
function readStoredComments(slug: string): CommentThread[] {
...
function writeStoredComments(slug: string, comments: CommentThread[]) {
  if (typeof window === "undefined") {
    return;
  }
...
export function NewsArticleInteractive({
  articleSlug,
  shareUrl,
  initialComments = [],
  children,
  ...
  useEffect(() => {
    const stored = readStoredComments(articleSlug);
    if (stored.length > 0) {
      setComments(stored);
    } else {
    ...
    setIsHydrated(true);
  }, [articleSlug, normalizedInitialComments]);

  useEffect(() => {
    if (!isHydrated) {
      return;
    }
    writeStoredComments(articleSlug, comments);
  }, [articleSlug, comments, isHydrated]);
      ...
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Compartilhe análises e impressões sobre esta reportagem.
            </p>
          </div>
          <span className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">
            {formattedInteractions} {totalInteractions === 1 ? "interação" : "interações"}
          </span>
        </div>
        ...
        {comments.length > 0 ? (
          <ul className="flex flex-col gap-6">
            {comments.map((comment) => {
              ...
              const formattedDate = commentDate
                ? commentDateFormatter.format(commentDate)
                : null;
                  ...
                  {activeReplyId === comment.id ? (
                    ...
                    >
                      <Textarea
                        ...
                        aria-label={`Responder ao comentário de ${comment.authorName}`}
                        required
                      />
          ...
          <div className="rounded-3xl border border-dashed border-border/70 bg-background/40 px-6 py-8 text-center text-muted-foreground">
            Seja o primeiro a comentar esta notícia e iniciar a conversa.
          </div>
        )}
      </section>
    </>
